...
def rename_files_enumerate(base_path):
    all_files = [y for x in os.walk(base_path) for y in glob(os.path.join(x[0], "*.*"))]
    for i, file in enumerate(tqdm(all_files, desc="Enumerating files and renaming")):
...
def convert_images_to_jpg(base_path):
    jpg_files = [
        y for x in os.walk(base_path) for y in glob(os.path.join(x[0], "*.jpg"))
    ]
    if jpg_files:
    ...
    jfif_files = [
        y for x in os.walk(base_path) for y in glob(os.path.join(x[0], "*.jfif"))
    ]
    if jfif_files:
    ...
    png_files = [
        y for x in os.walk(base_path) for y in glob(os.path.join(x[0], "*.png"))
    ]
    if png_files:
    ...
    files = [y for x in os.walk(base_path) for y in glob(os.path.join(x[0], "*.jpeg"))]
    if files:
...
def generate_tfrecord_files(tfrecords_path, images_path, images_per_file):
    ...
    print(
        "Pattern matches {} images which will be rewritten as {} TFRecord files containing ~{} images each.".format(
            found_images, math.ceil(found_images / images_per_file), images_per_file
        )
    )
    ...
    dataset = images.map(encode_image, num_parallel_calls=AUTO).batch(images_per_file)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate TFRecords from images")
    parser.add_argument(
        "--images-path",
        type=str,
